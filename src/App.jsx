...
const Header = ({ currentSection, onNavigate }) => {
  const getProgress = () => {
    // Calculate overall progress (placeholder - can be made dynamic)
    return 35;
  };
...
const MCQPractice = () => {
  ...
  const formatExplanation = (text) => {
    // Simple markdown-like formatting for tables and bold text
    const lines = text.split('\n');
    return lines.map((line, idx) => {
      if (line.includes('|')) {
        // Table row
        return <tr key={idx}><td>{line.split('|').slice(1, -1).join('|')}</td></tr>;
      }
